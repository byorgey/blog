...
<p>For a current project, I needed to implement type inference for a <a href="https://en.wikipedia.org/wiki/Hindley%E2%80%93Milner_type_system">Hindley-Milner</a>-based type system. (More about that project in an upcoming post!) If you don’t know, Hindley-Milner is what you get when you add polymorphism to the simply typed lambda calculus, but only allow $latex \forall$ to show up at the very outermost layer of a type. This is the fundamental basis for many real-world type systems (<em>e.g.</em> OCaml or Haskell without <code>RankNTypes</code> enabled).</p>
